...
/**
 * The gate for the generated ISO 15118-20 AC Kotlin codec: decode `expectedHex`, re-encode,
...
class VectorTest {
    ...
    private val vectors: List<JsonObject> by lazy {
        ...
        val f = File(dir, "libs/Vanaheimr.V2G.Exi/libs/WWCP_ISO15118/new/Vanaheimr.V2G.Exi.Tests/Vectors/Iso15118_20.AC.vectors.json")
    ...
    private fun reencode(m: Any): ByteArray = when (m) {
        is AC_ChargeParameterDiscoveryReq -> ACCodec.encode(m)
        is AC_ChargeParameterDiscoveryRes -> ACCodec.encode(m)
        is AC_ChargeLoopReq               -> ACCodec.encode(m)
        is AC_ChargeLoopRes               -> ACCodec.encode(m)
        is AC_CPDReqEnergyTransferMode    -> ACCodec.encode(m)
        is AC_CPDResEnergyTransferMode    -> ACCodec.encode(m)
        else -> error("no encode overload for ${m::class.simpleName}")
    }

    @Test
    fun `round-trips every AC vector through decode and re-encode`() {
        ...
        for (v in vectors) {
            ...
            val actual = try {
                reencode(ACCodec.decodeAny(expected))
            } catch (e: Throwable) {
        ...
        println("ISO 15118-20 AC vectors: ${vectors.size - failures.size}/${vectors.size} round-trip byte-exact")
