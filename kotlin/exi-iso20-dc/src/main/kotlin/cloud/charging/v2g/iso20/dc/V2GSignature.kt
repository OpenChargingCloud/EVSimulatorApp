...
/**
 * XMLDSig signing and verification for ISO 15118-20 DC.
 *
 ...
 * the whole set: SignedInfo is event code 129 at 8 bits here, against 230 at 9 bits in
 * CommonMessages. The same logical SignedInfo therefore signs different octets in each set, so
 * sharing one helper would mean signing the wrong bytes — a signature that verifies locally and
 * nowhere else.
 *
 * The element this set signs is `DC_ChargeParameterDiscoveryRes`, whose fragment codec the generator
...
object V2GSignature {
    ...
    fun signedInfoFragment(signedInfo: SignedInfoType): ByteArray =
        DCCodec.encodeFragment_SignedInfo(signedInfo)
