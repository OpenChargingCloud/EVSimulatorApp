...
/**
 * ISO 15118-20 AC_DER_IEC signing, over this message set's own XMLDSig types.
 *
...
class V2GSignatureTest {
    ...
    private fun signedElementFragment(power: Short = 22000): ByteArray =
        AcDerIecCodec.encodeFragment_AC_ChargeParameterDiscoveryRes(signedElement(power))
    ...
    fun `the SignedInfo fragment is this message set's own encoding`() {
        ...
        assertContentEquals(
            fragment,
            AcDerIecCodec.encodeFragment_SignedInfo(AcDerIecCodec.decodeFragment_SignedInfo(fragment)),
    ...
    fun `an Ed448 signature survives encode, decode and re-encode of the message`() {
        ...
        val bytes = AcDerIecCodec.encode(message)
        val decoded = AcDerIecCodec.decodeAny(bytes) as AC_ChargeParameterDiscoveryRes

        assertContentEquals(bytes, AcDerIecCodec.encode(decoded), "message round trip is not the identity")
