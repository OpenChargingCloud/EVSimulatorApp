...
/**
 * The gate for the generated ISO 15118-20 AC_DER_SAE Kotlin codec: decode `expectedHex`, re-encode,
...
class VectorTest {
    ...
    private val vectors: List<JsonObject> by lazy {
        ...
        val f = File(dir, "libs/Vanaheimr.V2G.Exi/libs/WWCP_ISO15118/new/Vanaheimr.V2G.Exi.Tests/Vectors/Iso15118_20.AC_DER_SAE.vectors.json")
    ...
    private fun reencode(m: Any): ByteArray = when (m) {
        is AC_ChargeParameterDiscoveryReq -> AcDerSaeCodec.encode(m)
        is AC_ChargeParameterDiscoveryRes -> AcDerSaeCodec.encode(m)
        is AC_ChargeLoopReq               -> AcDerSaeCodec.encode(m)
        is AC_ChargeLoopRes               -> AcDerSaeCodec.encode(m)
        is AC_CPDReqEnergyTransferMode    -> AcDerSaeCodec.encode(m)
        is AC_CPDResEnergyTransferMode    -> AcDerSaeCodec.encode(m)
        else -> error("no encode overload for ${m::class.simpleName}")
    }

    @Test
    fun `round-trips every AC_DER_SAE vector through decode and re-encode`() {
        ...
        for (v in vectors) {
            ...
            val actual = try {
                reencode(AcDerSaeCodec.decodeAny(expected))
            } catch (e: Throwable) {
        ...
        println("ISO 15118-20 AC_DER_SAE vectors: ${vectors.size - failures.size}/${vectors.size} round-trip byte-exact")
