...
/**
 * ISO 15118-20 AC_DER_SAE signing, over this message set's own XMLDSig types.
 *
...
class V2GSignatureTest {
    ...
    private fun signedElementFragment(power: Short = 22000): ByteArray =
        AcDerSaeCodec.encodeFragment_AC_ChargeParameterDiscoveryRes(signedElement(power))
    ...
    fun `the SignedInfo fragment is this message set's own encoding`() {
        ...
        assertContentEquals(
            fragment,
            AcDerSaeCodec.encodeFragment_SignedInfo(AcDerSaeCodec.decodeFragment_SignedInfo(fragment)),
    ...
    fun `an Ed448 signature survives encode, decode and re-encode of the message`() {
        ...
        val bytes = AcDerSaeCodec.encode(message)
        val decoded = AcDerSaeCodec.decodeAny(bytes) as AC_ChargeParameterDiscoveryRes

        assertContentEquals(bytes, AcDerSaeCodec.encode(decoded), "message round trip is not the identity")
