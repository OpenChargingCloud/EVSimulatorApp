...
namespace cloud.charging.open.protocols.ISO15118.EXI.Tests
{
    ...
    public class SwiftEmitterAttributeTests
    {
        ...
        {
            var doc = EmitterHarness.Emit(SwiftCodecEmitter.Instance, "test", "TestCodec", Schema(Required))
        ...
        {
            var doc = EmitterHarness.Emit(SwiftCodecEmitter.Instance, "test", "TestCodec", Schema(Optional))
                                    .Single(f => f.FileName == "Doc.swift").Source;
        ...
        {
            var swift  = CrossEmitterComparison.Operations(EmitterHarness.Emit(SwiftCodecEmitter.Instance, "test", "TestCodec", Schema(attrs)));
        ...
        public void RefusesARequiredAttributeAlongsideOthers()
        {
            ...
            var ex = Assert.Throws<NotSupportedException>(
                         () => EmitterHarness.Emit(SwiftCodecEmitter.Instance, "test", "TestCodec", Schema(mixed)));
        ...
        {
            var ex = Assert.Throws<NotSupportedException>(
                         () => EmitterHarness.Emit(SwiftCodecEmitter.Instance, "test", "TestCodec",
