...
/// The ISO 15118-20 AC_DER_IEC half of the -20 signature suite: SHA-512 over a signed element's EXI
...
/// set — so the *same* `SignedInfo` lands on a different event code in each. Here it is **217 over 9 bits**.
...
public enum V2GSignature {
    ...
    public static func sign(_ signedInfo: SignedInfoType,
        ...
        let fragment = AcDerIecCodec.encodeFragment_SignedInfo(signedInfo)
