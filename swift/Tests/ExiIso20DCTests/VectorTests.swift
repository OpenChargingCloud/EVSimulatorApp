import XCTest
@testable import ExiIso20DC

/// Gate 1 for the generated Iso15118_20.DC codec: decode `expectedHex`, re-encode, require the bytes
...
final class ExiIso20DCVectorTests: XCTestCase {
    ...
    private static func loadVectors() throws -> [(name: String, bytes: [UInt8])] {
        let url = try repositoryRoot().appendingPathComponent(
            "libs/Vanaheimr.V2G.Exi/libs/WWCP_ISO15118/new/Vanaheimr.V2G.Exi.Tests/Vectors/Iso15118_20.DC.vectors.json")
    ...
    private func reencode(_ m: Any) -> [UInt8]? {
        switch m {
        case let m as DC_ChargeParameterDiscoveryReq: return DCCodec.encode(m)
        case let m as DC_ChargeParameterDiscoveryRes: return DCCodec.encode(m)
        case let m as DC_CableCheckReq: return DCCodec.encode(m)
        case let m as DC_CableCheckRes: return DCCodec.encode(m)
        case let m as DC_PreChargeReq: return DCCodec.encode(m)
        case let m as DC_PreChargeRes: return DCCodec.encode(m)
        case let m as DC_ChargeLoopReq: return DCCodec.encode(m)
        case let m as DC_ChargeLoopRes: return DCCodec.encode(m)
        case let m as DC_WeldingDetectionReq: return DCCodec.encode(m)
        case let m as DC_WeldingDetectionRes: return DCCodec.encode(m)
        case let m as DC_CPDReqEnergyTransferMode: return DCCodec.encode(m)
        case let m as DC_CPDResEnergyTransferMode: return DCCodec.encode(m)
        default: return nil
        }
    }
    ...
    func testCorpusIsTheExpectedSize() throws {
        XCTAssertEqual(try Self.loadVectors().count, 16)
    }
    ...
    func testEveryVectorReEncodesToTheReferenceBytes() throws {
        ...
        for v in try Self.loadVectors() {
            do {
                let decoded = try DCCodec.decodeAny(v.bytes)
        ...
        XCTAssertTrue(failures.isEmpty,
                      "\(failures.count) of 16 vectors differ:\n" + failures.joined(separator: "\n"))
    }
}
